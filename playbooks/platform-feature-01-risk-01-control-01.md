...
2. Move plaintext literals to `confidential.yml`. This allows the plugin to generate obfuscated Swift accessors for those literals during the build. Note: The `confidential.yml` file should not be built into the app bundle.
...
4. Rebuild the app and use a static analysis tool, such as MobSF, to verify that the original plaintext literals are no longer easily recoverable from the `.ipa` file. Note: Swift Confidential prevents easy static extraction but not runtime attacks such as hooking, memory dumping, or patching.
...
The source code with the implemented control can be found [here](../implemented_controls/platform-feature-01-risk-01-control-01).
